...
2. `INBOUND_PARSE_WEBHOOK_PRIVATE_KEY` が未設定の場合、`email.rejected.no_webhook_signing_key` をログ出力し、`message.setReject('INBOUND_PARSE_WEBHOOK_PRIVATE_KEY not configured')` を呼び出して処理を終了します
3. `message.rawSize` が利用可能であり、かつ `MAX_MESSAGE_SIZE` を超過する場合、`email.rejected.too_large` をログ出力し、`message.setReject('Message too large')` を呼び出して処理を終了します

### 4.2 MIME 解析

本 Worker は、生のメールストリームを `postal-mime` を本線として解析します。

現行実装で行っている処理は以下のとおりです。

- 受信ストリーム全体を読み込み、`postal-mime` でパースする
- 生ヘッダを再解析し、RFC 2047 encoded-word を含むヘッダの復号結果を補完する
- 必要に応じて以下の論理ヘッダを抽出する
  - `from`
  - `to`
  - `cc`
  - `subject`
- multipart の場合は、構造異常判定に応じて互換フォールバック経路を選択する
...
1. 正常系（`postal-mime` 結果を採用）
  - `multipart/alternative` および単純な `multipart/mixed` では `postal-mime` の抽出結果を優先する
2. 異常シグナル検出時（互換フォールバックを適用）
  - boundary 欠落、開始/終端境界不整合、part 区切り不整合、複雑構造などを検出した場合にフォールバックへ切り替える
3. 互換フォールバック時の抽出
...
- 受信ストリーム全体を読み込み、`postal-mime` の抽出結果を利用する
- `text` に置換文字（`\uFFFD`）が含まれる場合は、`Content-Transfer-Encoding` と宣言済み文字コードを使って互換フォールバック復号を行う
...
署名対象は、timestamp 文字列の UTF-8 バイト列と、送信する exact raw multipart body bytes を連結した値です。
...
`text` / `html` は SendGrid Inbound Parse Webhook の Default Data Format と同じフォーム項目名で送信します。非 UTF-8 本文の場合も `textBytes` / `htmlBytes` のような独自項目は追加せず、`text` / `html` パートの値を元の本文文字コードに従うバイト列として送信します。
...
通常フォーム項目の multipart part header は、SendGrid の例に合わせて `Content-Disposition: form-data; name="..."` を基本とし、本文文字コードは `charsets` に格納します。
...
- `INBOUND_PARSE_WEBHOOK_PRIVATE_KEY` が未設定である場合
- `message.rawSize` が `MAX_MESSAGE_SIZE` を超過する場合
- 解析処理中に例外が発生した場合
- Webhook 署名生成中に例外が発生した場合

reject 理由は以下のいずれかです。

- `WEBHOOK_URL not configured`
- `INBOUND_PARSE_WEBHOOK_PRIVATE_KEY not configured`
- `Message too large`
- `Parsing error`
- `Webhook signing error`
...
- 解析例外発生時の reject
- Webhook 署名ヘッダの付与
- Webhook 署名対象と送信 multipart body の一致
...
- テスト: `test/index.spec.ts`, `test/mime-parser.spec.ts`, `test/postal-mime-adapter.spec.ts`
